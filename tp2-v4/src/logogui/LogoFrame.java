...
import myPackage.LogoSemanticVisitor;
...
public class LogoFrame extends JFrame {
	private int SIZE = 800, // for clearing view
			X = 1400, Y = SIZE;
	...
	private JSplitPane jlogoSplitPane = null; // split panel with logo program
												// and display
	...
	private JSplitPane jSplitASTPane = null; // split pane with ast and graphic
												// view
	private JScrollPane jScrollASTPane = null; // panel with AST tree
	private JTabbedPane jTabDisplayPane = null; // tabbed pane with ast and
												// graphic view
	...
	private boolean isViewVisible() {
		return tabbedview
				&& getJTabDisplayPanel().getSelectedComponent() == getJLogoPane()
				|| !tabbedview;
	}
	...
	private void initControlComponents() {
		...
		GroupLayout.ParallelGroup hButtonGroup = layout
				.createParallelGroup(Alignment.LEADING);
		hButtonGroup
				.addComponent(getJRunButton(), 0, GroupLayout.DEFAULT_SIZE,
						buttonDimension.width)
				.addComponent(getJClearButton(), 0, GroupLayout.DEFAULT_SIZE,
						buttonDimension.width)
				.addComponent(getJClearLogButton(), 0,
						GroupLayout.DEFAULT_SIZE, buttonDimension.width)
				.addComponent(getJChangeButton(), 0, GroupLayout.DEFAULT_SIZE,
						buttonDimension.width);

		GroupLayout.SequentialGroup vButtonGroup = layout
	...
	public JPanel getjTreeViewPane() {
		if (jTreeViewPane == null) {
			...
			scaleSlider = new JSlider(JSlider.HORIZONTAL, -999, 1000,
					sliderValue);
	...
	private void initFrameSize() {
		...
		setBounds((screenSize.width - xwin) / 2,
				(screenSize.height - ywin) / 2, xwin, ywin);
	}
	...
	private JScrollPane getJLogPane() {
		if (jLogPane == null) {
			...
			jLogPane.setBorder(javax.swing.BorderFactory.createTitledBorder(
					null, "Log",
	...
	private void runParser() {
		...
		if (program.length() > 1) {
			try {
				...
				// our code 
				ContextApplication.getMyContext().clear();
				LogoSemanticVisitor semVisitor = new LogoSemanticVisitor();
				
				Log.appendnl("***************************************************");
				semVisitor.visit(tree);

				if(!semVisitor.isExecutable()) {
					Log.appendnl("!----- ERREUR ! Fin d'analyse -----!");
					Log.appendnl("***************************************************");
				}
				else
				{
	...
	/*
	 * private TreeModel getDefaultTreeModel() { return new
	 * ASTtoTreeModelAdapter(null); }
	 */
